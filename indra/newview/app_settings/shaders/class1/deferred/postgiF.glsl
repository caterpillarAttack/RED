...
void main()
{
	vec3 norm = texture(normalMap, vary_fragcoord.xy).xyz;
	norm = vec3((norm.xy-0.5)*2.0,norm.z); // unpack norm
	vec3 pos = getPosition(vary_fragcoord.xy).xyz;

	vec3 ccol = texture(giLightMap, vary_fragcoord.xy).rgb;
	...
	for (int i = 0; i < kern_length; i++)
	{
		...
		vec3 sampNorm = texture(normalMap, tc.xy).xyz;
		...
		if (d > 0.8)
		{
			...
			{
	    		col += texture(giLightMap, tc).rgb*kern[i].x;
